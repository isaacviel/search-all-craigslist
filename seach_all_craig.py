...
class Search_All_Craiglist(unittest.TestCase):
    def setUp(self):
    ...
    def test_setup(self):
        ...
        query_params['model'] = raw_input("model?").replace(' ', '+')
        query_params['min_price'] = raw_input("min price?")
        query_params['max_price'] = raw_input("max price?")
        query_params['min_year'] = raw_input("min year?")
        query_params['max_year'] = raw_input("max year?")
        if raw_input("transmission?") == 'manual':
            query_params['transmission'] = str(1)
        else:
            query_params['transmission'] = str(2)

        query = 'search/cta?min_price=' + query_params['min_price'] + \
            '&max_price=' + query_params['max_price'] + \
            '&auto_make_model=' + query_params['model'] + \
            '&min_auto_year=' + query_params['min_year'] + \
            '&max_auto_year=' + query_params['max_year'] + \
            '&auto_transmission=' + query_params['transmission']

        print(query)

        results = []

        for x in us_cl_sites:
            driver.get(x + query)

            try:
                driver.find_element(
                    By.CSS_SELECTOR, '.alert.alert-sm.alert-warning')
            except:
                results.append(x + query)
